  ...
  const ModalPopup=()=>{
    ...
    {/* <Modal open={open} onClose={onCloseModal} center>
      <div style={{padding:"10px"}}>
            <h5 style={{fontWeight:"600"}}>Items already in cart</h5>
          <p style={{fontSize:"14px",lineHeight:"20px",color:"#535665",paddingBottom:"25px"}}>
          Your cart contains items from other restaurant.<br/> Would you like to reset your cart for adding items from this restaurant?
          </p>
          <div style={{display:"flex",justifyContent:"space-between"}}>
          <button style={{display:"flex",border:"2px solid #FFD438",fontWeight:"600",flexGrow:"1",justifyContent:"center",cursor:"pointer",background:"white",alignItems:"center",padding:"10px 20px",borderRadius:"20px"}} onClick={()=>setOpen(false)}>NO</button>
          <span style={{display:"inline-block",width:"20px",height:"100%"}}></span>
          <button style={{display:"flex",border:"2px solid #FFD438",fontWeight:"600",justifyContent:"center",cursor:"pointer",borderRadius:"20px",background:"#FFD438",alignItems:"center",padding:"0px 20px"}} onClick={()=>handleClearCart()}>YES,START AFRESH</button>
          </div>
          </div>
    </Modal> */}
    <Modal open={open} onClose={onCloseModal} center>
...
const mapStateToProps = (state) => {
  const { Menulist,menuObject } = state;
  return { Menulist,menuObject };
};
const actionCreator = {
